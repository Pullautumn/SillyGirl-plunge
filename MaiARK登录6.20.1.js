...
*/
    ...
    var addr = "http://192.168.18.249:5702"
    //这修改成自己MaiARK的ip地址和端口
    //最后面不要带“/” ，不然会出错！
    //只要修改这一处就行了，其他不懂就不要乱改！！！
     
    var user = GetUserID()
    sendText("Pullautumn为你服务，请输入11位手机号：(输入“q”随时退出会话。)");
    var num = input(20000)
...
function getcode() {
    ...
    if (!result) {
        ...
        sendText("获取验证码超时，请尝试重新登录，(如果一直此提示，请请联系管理员！)");
...
function LoginJD(result) {
    ...
    if (result1.msg == "验证码已过期，请重新获取") {
       sendText("验证码已过期，请重新登录！(如果一直此提示，请请联系管理员!)");
        return;
    }
